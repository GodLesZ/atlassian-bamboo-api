...
export default class Bamboo {
    ...
    getLatestSuccessfulBuildNumber(planKey, params, callback) {
        ...
        request({uri: planUri}, (error, response, body) => {
            ...
            if (newIndex < results.size) {
                self.getLatestSuccessfulBuildNumber(planKey, '?start-index=' + newIndex, (error, result) => {
    ...
    getAllPlans(params, callback, currentPlans) {
        ...
        request({uri: planUri}, (error, response, body) => {
            ...
            if (newIndex < plans.size) {
                self.getAllPlans('?start-index=' + newIndex, (error, result) => {
                    let errors = Bamboo.checkErrors(error, response);
     ...
     */
    getAllBuilds(params, callback, currentBuilds) {
        let self    = this,
        ...
        request({uri: planUri}, (error, response, body) => {
            ...
            for (let build of builds.result) {
                currentBuilds.push(build);
            }

            // Loop through the next series of builds
            let newIndex = builds['max-result'] + builds['start-index'];
            if (newIndex < builds.size) {
                self.getAllBuilds('&start-index=' + newIndex, (error, result) => {
                    let errors = Bamboo.checkErrors(error, response);
